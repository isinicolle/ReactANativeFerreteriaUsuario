...
const Pantalla = ({ navigation }) => {
  ...
  const fetchCiudades = async () => {
    if (selectedDepartamento.id_departamento)
      try {
        const res = await fetch('http://192.168.1.25:6001/api/ciudades/listarCiudades?idDepartamento=' + selectedDepartamento.id_departamento,
          {
            method: 'GET',
            headers: {
  ...
  const fetchDepartamentos = async () => {
    try {
      const res = await fetch('http://192.168.1.25:6001/api/departamentos/listar',
        {
          method: 'GET',
          headers: {
            Accept: 'application/json',
