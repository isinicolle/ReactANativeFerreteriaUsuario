...
const ConfiguracionDireccion = ({ navigation }) => {
    ...
    const cargar = async () => {
        fetch(buscarDireccion + idDireccion).then((response) => response.json())
            .then((json) => {
    ...
    const presGuardarCambio = async () => {
        try {
            const respuesta = await fetch(
                actualizarDireccion + idDireccion, {
                method: 'PUT',
                headers: {
                    Accept: 'application/json',
                    'Content-Type': 'application/json'
